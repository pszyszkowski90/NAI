...
Original file is located at
    https://colab.research.google.com/drive/1AyMf_12IJ5KJaBtxnDrpcfi0sHT1Nbqx
"""
...
num_pages = 2
output_dir = 'drive/MyDrive/Colab Notebooks/images2'
...
def downloader(link, genre, output_dir):
    ...
    savepath = '%s/%s/%s/%s' % (output_dir, genre, genre, filepath[-1])    
    try:
        time.sleep(0.2)  # try not to get a 403
        urllib.request.urlretrieve(file+'!PinterestSmall.jpg', savepath)
        num_downloaded += 1
...
if __name__ == '__main__':
    # if not os.path.isdir('%s/%s'%(output_dir+'/'+searchword, searchword)):
    searchword, typep = (genre, 'genre') if genre is not None else (style, 'style')
    try:
        os.mkdir('%s/%s'%(output_dir, searchword))
        os.mkdir('%s/%s/%s'%(output_dir, searchword,searchword))
    except  Exception as e:
        print('?')  
    num_pages = num_pages
